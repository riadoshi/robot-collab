...
def execute_action_plan(env, parser, obs, alice_action, bob_action, timeout=1500):
    ...
    for path_plan in path_plans:
        ...
        while not policy.plan_exhausted:
            ...
            stage_traj['video_imgs'].append(env.physics.render(camera_id='video', height=480, width=640))
            # stage_traj['ur5_wrist_imgs'].append(env.physics.render(camera_id='ur5_wrist_cam', height=480, width=640))
            # stage_traj['panda_wrist_imgs'].append(env.physics.render(camera_id='panda_wrist_cam', height=480, width=640))
            stage_traj['actions'].append(action)
            stage_traj['panda_states'].append(obs[0].panda.qpos)

            ur5_gripper_state = env.physics.named.data.qpos['right_driver_joint'] #env.physics.named.data.ctrl['robotiq_fingers_actuator']
            ur5_state = np.concatenate([obs[0].ur5e_robotiq.qpos[:6], ur5_gripper_state, [obs[0].ur5e_robotiq.qpos[6]]]) # arm, gripper, base
            stage_traj['ur5_states'].append(ur5_state)

            

    return True, stage_traj
...
def generate_demo(env, parser, demo_idx, save_dir, timeout=1500):
    ...
    try:
        ...
        for i, cube_name in enumerate(cube_names):
            ...
            success, stage_traj = execute_action_plan(
                env, parser, obs, 
                f"MOVE {cube_name}", 
                f"MOVE {cube_name}",
                timeout=timeout
            ...
            # Extend traj with info from execute_action_plan
            for k in stage_traj:
                traj[k].extend(stage_traj[k])
...
def main():
    ...
    parser_args.add_argument("--num_demos", "--n", type=int, default=75, help="Number of demos to generate")
    parser_args.add_argument("--save_dir", type=str, default="fixed_demos_sweep", help="Directory to save demos")
    ...
    for demo_idx in range(args.num_demos):
        ...
        try:
            ...
            if success:
                success_count += 1
                print("saving traj to npz file")
                np.savez(save_dir / f"traj_{demo_idx:03d}.npz", **traj)
        except Exception as e:
